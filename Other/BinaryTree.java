...
public class BinaryTree<T extends Comparable<T>> {
    ...
    private boolean tryAdd(Node newNode) {
        ...
        while (!stack.isEmpty()) {
            lowerMost = stack.pop();
            if (lowerMost.left != null)
                lowerMost.left = rebalance(lowerMost.left);
            if (lowerMost.right != null)
                lowerMost.right = rebalance(lowerMost.right);
    ...
    private Node rebalance(Node node) {
        ...
        else if (node.left != null && node.left.color == Color.RED && node.left.left != null
                && node.left.left.color == Color.RED)
            node = swapClockWise(node);

        else if (node.left != null && node.left.color == Color.RED && node.right != null
